...
export const TabItem = memo(function TabItem({
  ...
  if (!densityConfig) {
    ...
    console.log('TabItem rendered with density mode:', densityConfig.mode, 'for tab', tab.title?.substring(0, 30));
  }
  ...
  const containerStyle = useMemo(() => ({
    ...
    padding: densityConfig?.mode === 'compact' ? '6px 12px' :
             densityConfig?.mode === 'spacious' ? '12px' : '8px 12px',
    cursor: 'pointer',
  ...
  const handleImgError = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    e.currentTarget.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><rect width="16" height="16" fill="%23ddd"/><text x="8" y="12" text-anchor="middle" font-size="12" fill="%23666">?</text></svg>';
  }, []);
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%' }}>
        ...
        <div style={{ flex: 1, overflow: 'hidden', minWidth: 0 }} onClick={onClick}>
          ...
          {densityConfig?.showDomain && domain && (
            <div style={metaStyle}>{domain}</div>
          )}
          {densityConfig?.showUrl && tab.url && (
            <div style={metaStyle}>{tab.url}</div>
          )}
